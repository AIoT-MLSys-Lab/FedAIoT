...
from datasets.ego4d.utils import _get_frames
...
class StateChangeDetectionAndKeyframeLocalisation(torch.utils.data.Dataset):
    ...
    def __init__(self, mode):
        ...
        self.mode = mode
        self.ann_path = os.path.join('datasets/ego4d/ego4d_data_v2/v1/annotations', f'fho_oscc-pnr_{self.mode}.json')
        ann_err_msg = f"Wrong annotation path provided {self.ann_path}"
        assert os.path.exists(self.ann_path), ann_err_msg
        self.video_dir = 'datasets/ego4d/ego4d_data_v2/v1/video_540ss' # self.cfg.DATA.VIDEO_DIR_PATH
        assert os.path.exists(self.video_dir), "Wrong videos path provided"
        self.positive_vid_dir = 'datasets/ego4d/positive'
        ...
        self.negative_vid_dir = 'datasets/ego4d/negative'
    ...
    def get_frames_for(self, video_path, frames_list):
        ...
        """
        frames = list()
        with av.open(f'{video_path}.mp4') as container:
...
if __name__ == '__main__':
    # uids = []
    # ann_data = json.load(open('datasets/ego4d/ego4d_data/v1/annotations/fho_oscc-pnr_train.json', 'r'))['clips']
    # for i in ann_data:
    #     x = i['video_uid']
    #     uids.append(x)
    # ann_data = json.load(open('datasets/ego4d/ego4d_data/v1/annotations/fho_oscc-pnr_val.json', 'r'))['clips']
    # for i in ann_data:
    #     x = i['video_uid']
    #     # if not i['clip_uid']:
    #     uids.append(x)
    # uids = list(set(uids))
    # with open('datasets/ego4d/uids', 'w') as f:
    #     f.write(' '.join(uids))
    dataset = StateChangeDetectionAndKeyframeLocalisation('train')
