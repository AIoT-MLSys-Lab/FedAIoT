...
from loaders.visdrone import NAMES, YOLO_HYPERPARAMETERS
...
def evaluate(model, test_data, device, num_classes=12, batch_size=8):
    model.to(device)
    ...
    validator = YoloValidator(test_dataloader, args=YOLO_HYPERPARAMETERS, device=device,)
    #
    # losses = {'losses': LossMetric(Loss(de_parallel(model))).to(device)}
    # with torch.no_grad():
    ...
    return {k: v for k, v in metrics.items()} #| \
        # {f'Loss_{i}': l for i, l in enumerate(list(losses.values())[0].cpu().numpy())}
