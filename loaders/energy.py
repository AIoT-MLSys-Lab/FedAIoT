...
if __name__ == '__main__':
    ...
    print(len(dt['train']))
    print(len(dt['test']))
    print(dt['train'][0][0].shape, dt['train'][0][1])
    model = EnergyModel(input_size=dt['train'][0][0].shape[0], hidden_size=210, output_size=1)
    optimizer = optim.Adam(model.parameters(), lr=0.001, weight_decay=0.001)
    criterion = nn.MSELoss()
    train_loader = DataLoader(dataset=dt['train'], batch_size=128, shuffle=True)
    test_loader = DataLoader(dataset=dt['test'], batch_size=128, shuffle=False)
    # X_train, y_train, X_test, y_test = dt['dt']
    # Train the model
    num_epochs = 1000
    for epoch in range(num_epochs):
        ...
        with torch.no_grad():
            ...
            for inputs, targets in test_loader:
                ...
                targets = targets.to('cuda')
                output = model(inputs)
                predictions.extend(output.cpu().detach().numpy().tolist())
                actuals.extend(targets.cpu().numpy().tolist())

            # Compute R2 score
            r2_1 = r2_score(actuals, predictions)
            r2_2 = R2Score()(torch.tensor(predictions), torch.tensor(actuals).reshape((-1, 1)), )

            print(f'R2 Score: {r2_1}, {r2_2}')
    # random_forest = RandomForestRegressor(max_depth=26, n_estimators=211, random_state=42)
    #
    ...
    # forest_train_rmse = r2_score(y_train, forest_train_prediction)
    # forest_test_rmse = r2_score(y_test, forest_test_prediction)
